...
public class MainController implements Initializable, ITravelAgencyObserver {
    ...
    public void setService(ITravelAgencyServices serv) { this.server = serv; }
    ...
    public void tripReserved(Reservation reservation) throws TravelAgencyException {
        Platform.runLater(() -> {
            for(Trip trip : tripsTableView.getItems()) {
                if (trip.getId() == reservation.getTrip().getId()) {
                    int newNumber = trip.getAvailableSeats() - reservation.getNumberOfTickets();
